 ...
 * These bones are to be used to modify the vertices of submesh for skeletal
 * animation.
 * <p>
 * Within the submesh, each vertex has up to 3 bone parents and corresponding
 * weights that determine
 * the influence that up to 3 bones have on said vertex. This is kept track
 * through the vertex
 * having 3 bone indices that directly correlate to data stored in various
 * buffers within the Skeleton class.
 * <p>
 ...
 * Instead, it should use the loadAnimation() and playAnimation() tools in
 * AnimationShape.
 * 
...
public final class AnimatedSkeleton {
	...
	// The rest position of each bone ( a list of 3D vectors, relative parent bone
	// tail)
	...
	protected void setBoneCount(int boneCount) {
	...
	protected void setBoneNames(String[] bn) {
		boneNames = bn;
	}
	...
	protected void setBoneLengthsBuffer(FloatBuffer bl) {
		boneLengthsBuffer = bl;
	}

	protected void setBoneRestRotationsBuffer(FloatBuffer brr) {
		boneRestRotationsBuffer = brr;
	}

	protected void setBoneRestLocationsBuffer(FloatBuffer brl) {
		boneRestLocationsBuffer = brl;
	}

	protected void setBoneParentsBuffer(IntBuffer bp) {
	...
	// Returns the rest location vector3 of the bone with the index boneIndex in
	// this skeleton's data.
	...
	protected float getBoneLength(int boneIndex) {
		if (boneIndex < 0 || boneIndex > boneCount)
			throw new IndexOutOfBoundsException(
					"Attempted to get bone length of out of bounds bone " + boneIndex + ".");
