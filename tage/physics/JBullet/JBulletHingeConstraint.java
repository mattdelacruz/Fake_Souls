...
public class JBulletHingeConstraint extends JBulletConstraint implements PhysicsHingeConstraint {
	...
	public JBulletHingeConstraint(int uid, JBulletPhysicsObject bodyA, JBulletPhysicsObject bodyB, float axisX,
		...
		float[] pivotInB = new float[] { (float) (bodyA.getTransform()[12] - bodyB.getTransform()[12]),
				(float) (bodyA.getTransform()[13] - bodyB.getTransform()[13]),
				(float) (bodyA.getTransform()[14] - bodyB.getTransform()[14]) };
		axis = new float[] { axisX, axisY, axisZ };
		hingeConstraint = new HingeConstraint(rigidA, rigidB, new Vector3f(pivotInA), new Vector3f(pivotInB),
				new Vector3f(axisX, axisY, axisZ), new Vector3f(axisX, axisY, axisZ));
